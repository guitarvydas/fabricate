...
function compilefmt (fmtsrc, ohmlang) {
    ...
    if (fmtcst.failed ()) {
        // return [false, "FMTRICATOR: syntax error\n(Use Ohm-Editor to debug fmtricator specification (grammar: fmt.ohm))\n\n" + internalgrammar.trace (fmtsrc)];
	console.error (internalgrammar);
        return [false, "FMTRICATOR: syntax error\n(Use Ohm-Editor to debug fmtricator specification) rightmostPosition=" + fmtcst.getRightmostFailurePosition() + '\n' + fmtsrc];
    }
